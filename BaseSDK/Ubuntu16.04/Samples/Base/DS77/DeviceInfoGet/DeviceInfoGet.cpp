...
int main()
{
	...
	char fw[63] = { 0 };
	status = scGetFirmwareVersion(deviceHandle, fw, 63);
	if (status == ScStatus::SC_OK)
	{
		cout << "[scGetFirmwareVersion] success, ScStatus(" << status << "). The device firmware version is " << fw << endl;
	}
	else
	{
		cout << "[scGetFirmwareVersion] fail, ScStatus(" << status << ")." << endl;
		return -1;
	}
