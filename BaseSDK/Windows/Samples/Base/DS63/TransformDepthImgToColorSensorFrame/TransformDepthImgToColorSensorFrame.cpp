...
int main()
{
	...
	if (status == ScStatus::SC_OK)
	{
		cout << "[scSetTransformDepthImgToColorSensorEnabled] success, ScStatus(" << status << ")." << endl;
	}
	else
	{
		...
		if (1 == frameReady.transformedDepth)
		{
			...
			if (status == ScStatus::SC_OK)
			{
				...
				if(!saveonce)
				{
					...
					framename << "TransformDepthImgToColorSensorFrame_" << frame.frameIndex << ".bin";
					char fname[64];
					framename >> fname;
					ofstream file;
					...
					file.close();
					cout << "Save TransformDepthImgToColorSensorFrame successful in " << fname << endl;
					saveonce = true;
				}
			}
			else
