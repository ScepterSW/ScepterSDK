...
GET:
    ...
    status = scOpenDeviceByIP(pPsDeviceInfo->ip, &deviceHandle_);
    if (status != ScStatus::SC_OK)
    {
        ROS_INFO("Ip %s OpenDevice failed! status %d", pPsDeviceInfo->ip, status);
    }
    else
    {
...
void ScepterManager::run() 
{
    if (deviceHandle_ == nullptr)
    {
        ROS_INFO_STREAM( "Pending the process as deviceHandle is null for " << camera_ip_ );
        ros::spin();
    }
    else
    {
...
void ScepterManager::set_sensor_intrinsics()
{
    ...
    ROS_INFO("Successfully received intrinsic and extrinsic parameters for device %s",  this->camera_ip_.c_str());
