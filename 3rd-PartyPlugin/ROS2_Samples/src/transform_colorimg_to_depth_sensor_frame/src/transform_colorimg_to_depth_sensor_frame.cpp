/*
    This client is used to demo how to subscribe to the transformed color image.
*/
...
rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr transformedColorImageSub_;

class TransformColorImgToDepthSensorFrameNode:public rclcpp::Node {
public:
        TransformColorImgToDepthSensorFrameNode(std::string nodeName, rclcpp::NodeOptions& node_options):Node(nodeName, node_options.automatically_declare_parameters_from_overrides(true))
        {
            RCLCPP_INFO(this->get_logger(), "TransformColorImgToDepthSensorFrame");
        ...
        bool connect_server() {
            while(!asyncParam_client_->wait_for_service(1s))//wait for server 1s
            {
                if(!rclcpp::ok())
                {
                    RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "stop client!");
                    return false;
                }
                RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "async connecting server......");
            }
            return true;
        }
...
{
    frameIndex++;
    ssm << "./src/transform_colorimg_to_depth_sensor_frame/";
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "exit subscribe image ");
        isGetRes = true;
...
void subscribeTransformedColorImage(std::shared_ptr<TransformColorImgToDepthSensorFrameNode>& node)
{
    if (node == nullptr)
    {
        isGetRes = true;
    ...
    RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "subscribeTransformedColorImage");
    node->getClient()->set_parameters({
        rclcpp::Parameter("framerate",10),
        rclcpp::Parameter("transformed_color",true),
        rclcpp::Parameter("work_mode",0)
    });
    std::string transformedColorTopic = node->getNodeName() + "/" + node->getCameraSN() + std::string("/transformedColor/image_raw");
#ifdef FOXY
...
#endif
    transformedColorImageSub_ = node->create_subscription<sensor_msgs::msg::Image>(transformedColorTopic.c_str(), qosVol, image_callback);
    return;
}

bool getCameraSN(std::shared_ptr<TransformColorImgToDepthSensorFrameNode>& node)
{
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<TransformColorImgToDepthSensorFrameNode>("TransformColorImgToDepthSensorFrame", node_options);
    bool flag = node->connect_server();
    if(!flag)
    {
        RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "connect server failed");
        return -1;
    }
    ...
    if (ret == false)
    {
        RCLCPP_INFO(rclcpp::get_logger("TransformColorImgToDepthSensorFrame"), "getCameraSN failed");
        return -1;
    }
    isGetRes = false;
    rclcpp::executors::SingleThreadedExecutor executor;
    executor.add_node(node);
    subscribeTransformedColorImage(node);
    while(isGetRes == false)
    {
        executor.spin_some();
        loop_rate.sleep();
    }
    transformedColorImageSub_.reset();
    rclcpp::shutdown();
    return 0;
}
