/*
    This client is used to demo how to subscribe to the depth2color cloud point.
*/
...
rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr depth2colorCloudSub_;

class PointCloudCaptureAndSaveDepthImgToColorSensorNode:public rclcpp::Node {
public:
        PointCloudCaptureAndSaveDepthImgToColorSensorNode(std::string nodeName, rclcpp::NodeOptions& node_options):Node(nodeName, node_options.automatically_declare_parameters_from_overrides(true))
        {
            RCLCPP_INFO(this->get_logger(), "PointCloudCaptureAndSaveDepthImgToColorSensor");
        ...
        bool connect_server() {
            while(!asyncParam_client_->wait_for_service(1s))//wait for server 1s
            {
                if(!rclcpp::ok())
                {
                    RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "stop client!");
                    return false;
                }
                RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "async connecting server......");
            }
            return true;
        }
...
void cloudPoint_callback(const sensor_msgs::msg::PointCloud2::SharedPtr cloudPoint)
{
    ...
    ssm << "./src/point_cloud_capture_and_save_depthimg_to_color_sensor/";
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "exit point cloud");
        isGetRes = true;
    }
    return;
}

void subscribeDepth2ColorCloudPoint(std::shared_ptr<PointCloudCaptureAndSaveDepthImgToColorSensorNode>& node)
{
    if (node == nullptr)
    {
        isGetRes = true;
    ...
    RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "subscribeDepth2ColorCloudPoint");
    node->getClient()->set_parameters({
        rclcpp::Parameter("framerate",10),
        rclcpp::Parameter("transformed_depth",true),
        rclcpp::Parameter("depth2color_cloud_point",true),
        rclcpp::Parameter("work_mode",0)
    });
    std::string depth2colorCloudPointTopic = node->getNodeName() + "/" + node->getCameraSN() + std::string("/depth2color/points");
#ifdef FOXY
...
#endif
    depth2colorCloudSub_ = node->create_subscription<sensor_msgs::msg::PointCloud2>(depth2colorCloudPointTopic.c_str(), qosVol, cloudPoint_callback);
    return;
}

bool getCameraSN(std::shared_ptr<PointCloudCaptureAndSaveDepthImgToColorSensorNode>& node)
{
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<PointCloudCaptureAndSaveDepthImgToColorSensorNode>("PointCloudCaptureAndSaveDepthImgToColorSensor", node_options);
    bool flag = node->connect_server();
    if(!flag)
    {
        RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "connect server failed");
        return -1;
    }
    ...
    if (ret == false)
    {
        RCLCPP_INFO(rclcpp::get_logger("PointCloudCaptureAndSaveDepthImgToColorSensor"), "getCameraSN failed");
        return -1;
    }
    isGetRes = false;
    rclcpp::executors::SingleThreadedExecutor executor;
    executor.add_node(node);
    subscribeDepth2ColorCloudPoint(node);
    while(isGetRes == false)
    {
        executor.spin_some();
        loop_rate.sleep();
    }
    depth2colorCloudSub_.reset();
    rclcpp::shutdown();
    return 0;
