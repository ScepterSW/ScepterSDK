...
ScepterManager::ScepterManager(int32_t device_index,std::string camera_name) :
        Node(camera_name),
        ...
        rcl_interfaces::msg::SetParametersResult result;
            ...
            for (const auto &param : params) {
                if (param.get_name() == "DepthCloudPointFlag") {
                    RCLCPP_INFO(this->get_logger(), "DepthCloudPointFlag updated: %d", param.as_bool());
                    depthCloudPointFlag_ = param.as_bool();
                }
                if (param.get_name() == "Depth2ColorCloudPointFlag") {
                    RCLCPP_INFO(this->get_logger(), "Depth2ColorCloudPointFlag updated: %d", param.as_bool());
...
bool ScepterManager::initDCAM(int32_t device_index)
{ 
    ScStatus status = ScStatus::SC_OK;
    // Initialise the API
    status =scInitialize();
