...
namespace DeviceIPAndSubnetMaskSet
{
    ...
    class Program
    {
        static int Main(string[] args)
        {
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetDeviceDHCPEnabled] success status:(" + status + "). The device DHCP is close.");
            }
            else
            {
                Console.WriteLine("[VN_SetDeviceDHCPEnabled] fail status:(" + status + ").");
                return 1;
            }
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetDeviceIPAddr] success status:(" + status + "). Set the device IP to " + strIP);
            }
            else
            {
                Console.WriteLine("[VN_SetDeviceIPAddr] fail status:(" + status + ").");
                return 1;
            }
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetDeviceSubnetMask] success status:(" + status + "). Set the device subnet mask to 255.255.255.0 ");
            }
            else
            {
                Console.WriteLine("[VN_SetDeviceSubnetMask] fail status:(" + status + ").");
                return 1;
            }
