...
namespace DeviceSetParamsByJson
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceSetParamsByJson---");
            ...
            {
                Console.WriteLine("Please input JSON file path: ");
                pImgPath = Console.ReadLine();
            }

            //Set the parameters by Json file
            //If fails, please refer to the log file of ScenseSDK.txt
            status = VNAPI.VN_SetParamsByJson(deviceHandle, pImgPath);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetParamsByJson] success status:(" + status + ").");
            }
            else
            {
                Console.WriteLine("[VN_SetParamsByJsons] fail status:(" + status + ").");
                return 1;
            }
