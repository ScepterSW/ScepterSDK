...
namespace DeviceConnectByIP
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceConnectByIP---");
            ...
            status = VNAPI.VN_OpenDeviceByIP(pDeviceListInfo[0].ip, ref deviceHandle);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_OpenDeviceByIP] success status:(" + status + ").");
            }
            else
            {
                Console.WriteLine("[VN_OpenDeviceByIP] fail status:(" + status + ").");
                return 1;
            }
