...
namespace DeviceSetPTP
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceSetPTP---");
            ...
            tScTimeSyncConfig.flag = 2;
            status = VNAPI.VN_SetRealTimeSyncConfig(deviceHandle, tScTimeSyncConfig);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetRealTimeSyncConfig] success status:(" + status + "). Set the device PTP time synchronization success. ");
            }
            else
            {
