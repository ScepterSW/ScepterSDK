...
namespace DeviceSetSoftwareTriggerParameter
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceSetSoftwareTriggerParameter---");
            ...
            status = VNAPI.VN_SetSoftwareTriggerParameter(deviceHandle, 5);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetSoftwareTriggerParameter] success status:(" + status + "). Set the device software trigger parameter to 5.");
            }
            else
            {
                Console.WriteLine("[VN_SetSoftwareTriggerParameter] fail status:(" + status + ").");
                return 1;
            }
