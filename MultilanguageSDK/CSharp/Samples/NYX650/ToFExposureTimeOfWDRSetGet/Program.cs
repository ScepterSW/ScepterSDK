...
namespace ToFExposureTimeOfWDRSetGet
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (status != ScStatus.SC_OK)
            {
                Console.WriteLine("scGetWDRModeEnabled failed status:" + status);
                return;
            }
            Console.WriteLine("Get WDRMode status:" + enabled);
            ...
            if (0 == enabled)
            {
                ...
                if (status != ScStatus.SC_OK)
                {
                    Console.WriteLine("scSetWDRModeEnabled failed status:" + status);
                    return;
                }
                Console.WriteLine("Open WDR mode");
            }
            ...
            if (ScStatus.SC_OK == status)
            {
                for (int i = 0; i < nCount; i++)
                {
                    ...
                    if (status != ScStatus.SC_OK)
                    {
                        Console.WriteLine("scGetMaxExposureTimeOfWDR FrameCount: " + i + " failed status:" + status);
                        return;
                    }
                    ...
                    if (status != ScStatus.SC_OK)
                    {
                        Console.WriteLine("scGetExposureTimeOfWDR FrameCount: " + i + " failed status:" + status);
                        return;
                    }
                    ...
                    if (status != ScStatus.SC_OK)
                    {
                        Console.WriteLine("scSetExposureTimeOfWDR FrameCount: " + i + " failed status:" + status);
                        return;
                    }
            ...
            {
                Console.WriteLine("scGetFrameCountOfWDRMode failed status:" + status);
                return;
            }
