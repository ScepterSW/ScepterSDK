...
namespace DeviceSetNTP
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceSetNTP---");
            ...
            ////Set NTP
            ScTimeSyncConfig tScTimeSyncConfig = new ScTimeSyncConfig();
            tScTimeSyncConfig.ip = new byte[16];
            tScTimeSyncConfig.flag = 1;
            string str = "192.168.1.100\0";
            byte[] bytes = Encoding.UTF8.GetBytes(str);
            Buffer.BlockCopy(bytes, 0, tScTimeSyncConfig.ip, 0, 14);
            status = VNAPI.VN_SetRealTimeSyncConfig(deviceHandle, tScTimeSyncConfig);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetRealTimeSyncConfig] success status:(" + status + "). Set the device NTP server IP to " + str);
            }
            else
            {
                Console.WriteLine("[VN_SetRealTimeSyncConfig] fail status:(" + status + ").");
                return 1;
            }
