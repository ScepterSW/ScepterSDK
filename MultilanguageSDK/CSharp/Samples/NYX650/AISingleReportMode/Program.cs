...
namespace AISingleReportMode
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---AISingleReportMode---");
            ...
            for (int j = 0; j < 10; j++)
            {
                ...
                if (status == ScStatus.SC_OK)
                {
                    Console.WriteLine("[VN_AIModuleTriggerOnce] success status:(" + status + ").");
                }
                else
                {
                ...
                if (status == ScStatus.SC_OK)
                {
                    Console.WriteLine("[VN_AIModuleGetResult] success status:(" + status + "). ScAIResult <resultIndex>: " + aiResult.resultIndex);
                }
                else
                {
        ...
        public static ScStatus SetParams(ScDeviceHandle deviceHandle, ref ScepterAPI VNAPI)
        {
            ...
            status = VNAPI.VN_AIModuleSetWorkMode(deviceHandle, ScAIModuleMode.AI_SINGLE_REPORT_MODE);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_AIModuleSetWorkMode] success status:(" + status + "). Set AI_SINGLE_REPORT_MODE.");
            }
            else
            {
