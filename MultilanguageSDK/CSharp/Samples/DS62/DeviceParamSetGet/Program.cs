...
namespace DeviceParamSetGet
{
    ...
    class Program
    {
        static int Main(string[] args)
        {
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_GetIRGMMGain] success status:(" + status + "). The device IRGMM gain is "+ (int)gmmgain);
            }
            else
            {
                Console.WriteLine("[VN_GetIRGMMGain] fail status:(" + status + ").");
                return 1;
            }
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetIRGMMGain] success status:(" + status + "). Set the device IRGMM gain to "+ (int)gmmgain);
            }
            else
            {
                Console.WriteLine("[VN_SetIRGMMGain] fail status:(" + status + ").");
                return 1;
            }
            //1.close device
            //2.SDK shutdown
