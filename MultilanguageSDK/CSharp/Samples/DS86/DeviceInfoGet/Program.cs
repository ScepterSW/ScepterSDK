...
namespace DeviceInfoGet
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---DeviceInfoGet---");
            ...
            //get FW
            IntPtr cFirmwareVersion = Marshal.AllocHGlobal(63);
            status = VNAPI.VN_GetFirmwareVersion(deviceHandle, cFirmwareVersion, 63);
            string strFirmwareVersion = Marshal.PtrToStringAnsi(cFirmwareVersion, 63);
            Marshal.FreeHGlobal(cFirmwareVersion);
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_GetFirmwareVersion] success status:(" + status + "). The device firmware version is: " + strFirmwareVersion);
            }
            else
            {
                Console.WriteLine("[VN_GetFirmwareVersion] fail status:(" + status + ").");
                return 1;
            }
