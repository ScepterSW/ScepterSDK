...
namespace DeviceSWTriggerMode
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("---AISingleTriggerResultDelayTest---");
            ...
            try
            {
                using (StreamWriter sWriter = new StreamWriter("AISingleTriggerResultDelayTest.csv"))
                {
                    sWriter.Close();
                }
            }
            ...
            StreamWriter csvWriter = new StreamWriter("AISingleTriggerResultDelayTest.csv");
...
GOTOTAG:
                ...
                string pResultData = Marshal.PtrToStringAnsi(pAIResult.pResultData, (int)pAIResult.dataLen);
                if (pResultData.IndexOf("{") == -1)
                {
                    Console.WriteLine("The last useless res: " + pAIResult.resultIndex);
                    goto GOTOTAG;
                }
                ...
                if (0 == pAIResult.resultIndex % 10)
                {
                    Console.WriteLine("VN_GetFrame,status:" + status + "  "
                        + "pAIResult resultIndex:" + pAIResult.resultIndex);
                }
