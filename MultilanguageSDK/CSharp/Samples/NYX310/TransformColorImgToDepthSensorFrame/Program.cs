...
namespace TransformColorImgToDepthSensorFrame
{
    ...
    class Program
    {
        static int Main(string[] args)
        {
            ...
            if (status == ScStatus.SC_OK)
            {
                Console.WriteLine("[VN_SetTransformColorImgToDepthSensorEnabled] success status:(" + status + ").");
            }
            else
            {
                Console.WriteLine("[VN_SetTransformColorImgToDepthSensorEnabled] fail status:(" + status + ").");
                return 1;
            }
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                {
                    status = VNAPI.VN_GetFrame(deviceHandle, ScFrameType.SC_TRANSFORM_COLOR_IMG_TO_DEPTH_SENSOR_FRAME, ref frame);
                    if (status == ScStatus.SC_OK && frame.pFrameData != IntPtr.Zero)
                    {
                        Console.WriteLine("[VN_GetFrame] success status:(" + status + "). SC_TRANSFORM_COLOR_IMG_TO_DEPTH_SENSOR_FRAME <frameIndex>: " + frame.frameIndex);

                        if (!saveonce)
                        { //name of the frames to be saved.
                            string fname = "TransformColorImgToDepthSensorFrame_" + frame.frameIndex + ".bin";
                            ...
                            try
                            {
                                ...
                                sw.Flush();
                                sw.Close();
                                Console.WriteLine("Save TransformColorImgToDepthSensorFrame successful in " + fname);
                                saveonce = true;
                            }
                            catch (Exception e)
                            {
