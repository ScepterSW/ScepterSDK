...
import time
print('---DeviceImportParamInitFile---')
...
if  ret == 0:
    ...
    exit(1)
imgpath ="./camera_config.json"
if not os.path.exists(imgpath):
    imgpath = input('Please input init file path: ')

ret = camera.scImportParamInitFile(imgpath)
if  ret == 0:
    print('[scImportParamInitFile] success ScStatus({}).'.format(str(ret)))
    ...
    print('[scImportParamInitFile] fail ScStatus({}).'.format(str(ret)))
    exit(1)
