...
frameSpace = 20
print("---TransformDepthImgToColorSensorFrame---")
...
if  ret == 0:
    print('[scSetTransformDepthImgToColorSensorEnabled] success ScStatus({}).'.format(str(ret)))
else:
...
for i in range(frameSpace):
    ...
    if  frameready.transformedDepth:      
        ...
        if  ret == 0:
            ...
            if saveonce == False:
                curPath = os.getcwd()
                filename = "TransformDepthImgToColorSensorFrame_{}.bin".format(str(frame.frameIndex))
                file = open(curPath + "//" +filename,"wb+")
                for i in range(frame.dataLen):
                    file.write(c_uint8(frame.pFrameData[i]))
                file.close()
                print("Save TransformDepthImgToColorSensorFrame successful in {}".format(filename))
                saveonce = True
        else:   
